...
| 8 表紙作成 | タイトル・帯コピーを固め、`image_gen` で文字入り版・文字なし版を生成後、Amazon向けJPEGへ書き出す | Step0（タイトル）＋Step3（参照画像・art_style） | `step8_表紙/[作品名]_step8_cover_prompt.md` ＋ `step8_表紙/[作品名]_cover_with_text.jpg` ＋ `step8_表紙/[作品名]_cover_no_text.jpg` |
...
7. `image_gen`の生成画像（通常はPNG）を一時ファイルとして保存後、Amazon向けの納品JPEGへ書き出す。歪みを避けるため、高さ2560pxへ拡大してから左右を中央トリミングし、幅1600pxにする。
   ```bash
   bash scripts/export_amazon_cover.sh "$with_text_png" "step8_表紙/[作品名]_cover_with_text.jpg"
   bash scripts/export_amazon_cover.sh "$no_text_png" "step8_表紙/[作品名]_cover_no_text.jpg"
   ```
   作り直しは `..._v2_...` で別名保存（明示時のみ上書き）。
8. 文字入り版の日本語が指定どおり読めるか、両JPEGが**1600×2560ピクセル・JPEG形式**であるか、キャラの主要特徴を確認する。完了報告：使用した参照シート・生成方式（文字入り完成版＋文字なし安全版）・両方の保存パス・確認した実寸・Canva等での文字配置一覧・日本語文字崩れがある場合の注意。
...
│   ├── [作品名]_cover_with_text.jpg
│   └── [作品名]_cover_no_text.jpg
└── scripts/
    ├── prepare_pages.py              ← 参照画像候補と保存先を出す汎用ローダー（Codexが実行）
    └── export_amazon_cover.sh         ← 表紙を1600×2560 JPEGへ書き出す
```
